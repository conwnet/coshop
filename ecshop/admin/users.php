...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, parent_id ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        ...
        $sql = "SELECT user_name FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id=" . $user_list[$i]['parent_id'];
        $mobile = $GLOBALS['db']->getOne($sql);
        $user_list[$i]['referee'] = $mobile;
    }
