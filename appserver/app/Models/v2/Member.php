...
class Member extends BaseModel {
    ...
    protected $guarded = [];
    protected $appends = ['id','age','rank','gender','username','nickname','mobile','avatar','mobile_binded','joined_at','is_auth', 'is_completed', 'info', 'is_qiandao'];
    protected $visible = ['id','age','rank','gender','username','nickname','mobile','avatar','mobile_binded','joined_at','is_auth', 'is_completed', 'info', 'is_qiandao'];
    ...
    public static function createMemberByMobile(array $attributes)
    {
        ...
        {
            /// 临时去掉短信验证码验证，方便注册
            if (false && !self::verifyCode($mobile, $code)) {
                return self::formatError(self::BAD_REQUEST, trans('message.member.mobile.code.error'));
            }

            $parent = Member::where('user_name', $referee)->first();
            $data = [
                'user_name' => $mobile,
            ...
            if ($parent) $data['parent_id'] = $parent['user_id'];
            ...
            if ($model = self::create($data))
            {
                ...
                /// return self::formatError(1, );

                return self::formatBody(['token' => $token /*, 'user' => $model->toArray() 有趣的 bug 不知道怎么回事 */]);
    ...
    public function getInfoAttribute() {
        $msn = $this->attributes['msn'];
        return unserialize($msn ? $msn : 'a:0:{}');
    }

    public function getIsQiandaoAttribute()
    {
        $record = AccountLog::where('change_desc', '签到获取')->where('user_id', $this->attributes['user_id'])->orderBy('change_time', 'DESC')->first();
        return $record && date('Y-m-d', $record->change_time) == date('Y-m-d', time());
    }
}
